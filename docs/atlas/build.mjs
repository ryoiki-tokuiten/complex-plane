...
// Usage: node docs/atlas/build.mjs
...
const cnt = { open: 0, res: 0, to: 0 };
NODES.forEach((n) => (n.cond || []).map(Q).forEach((c) => {
  if (c.r) cnt.res++;
  else if (c.to) cnt.to++;
  else cnt.open++;
}));
...
function buildSystemMd() {
  ...
  out.push(META.intro, '');
  out.push(`_Question status: **${cnt.open} open · ${cnt.to} routed · ${cnt.res} resolved**._`, '');
  ...
  out.push('');
  if (META.costModel && META.costModel.length) {
    out.push('## Performance and Resource Model', '');
  ...
  out.push('## Flows (representative packets)', '', 'Payload shapes represent the real runtime contracts and state objects passed across modules.', '');
  ...
  out.push('## Questions — index', '', 'Reference by ID. ✓ resolved · → routed · otherwise open.', '');
  index.forEach(([id, code, c]) => {
    if (c.r) out.push(`- ~~**${id}**~~ (${code}) ✓ ${md(c.r)}`);
    else if (c.to) out.push(`- **${id}** (${code}) → _${md(c.to)}_ (${md(c.q)})`);
    else out.push(`- **${id}** (${code}) ${md(c.q)}`);
  });
  out.push('');
  ...
  if (META.filesystem) out.push('## Filesystem layout', '', '```', META.filesystem.trimEnd(), '```', '');
  out.push('## How this file is maintained', '', `Generated from \`${META.sourcePath || 'docs/atlas/data.mjs'}\` by \`${META.buildCmd || 'node docs/atlas/build.mjs'}\`, which also builds the interactive atlas (\`atlas.html\`${META.artifactUrl ? `, published at ${META.artifactUrl}` : ''}). Edit the data file, rebuild, republish — never edit this file by hand.`, '');
  return out.join('\n');
}
