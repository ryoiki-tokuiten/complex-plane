...
test('native map dynamics evaluator matches current mapped output-chain semantics', () => {
    const before = snapshotState();
...
test('native map algebraic output chains match the main domain-coloring pipeline across modes', () => {
    const before = snapshotState();
    const plane = {
        width: 9,
        height: 7,
...
test('native map zeta continuation chains match the main mapped transform in the critical strip', () => {
    const before = snapshotState();
    const plane = {
        width: 8,
        height: 8,
...
test('domain dispatch waits for WASM worker readiness and commits full-size current frames', () => {
    ...
    const plane = { width: 4, height: 3, currentVisXRange: [-1, 1], currentVisYRange: [-1, 1] };
    const target = { save() {}, restore() {}, setTransform() {}, clearRect() {}, drawImage(...args) { draws.push(args); } };
    let closed = 0;
    const bitmap = { close() { closed++; } };
    try {
        cancelPlanarDomainDynamics();
        const snapshot = buildPlanarDomainDynamicsSnapshot(state, plane);
        renderPlanarDomainDynamics(target, plane, snapshot);
        const worker = workers[0];
        assert.equal(worker.messages.length, 0);
        worker.receive({ type: 'ready' });
        assert.equal(worker.messages.length, 1);
        const jobId = worker.messages[0].jobId;
        worker.receive({ type: 'frame', jobId, bitmap, stats: { completedSamples: 48 } });
        assert.deepEqual(draws, [[bitmap, 0, 0]]);
        assert.equal(closed, 1);
        const next = { ...snapshot, viewport: { ...snapshot.viewport, centerRe: '2' } };
        renderPlanarDomainDynamics(target, plane, next);
        assert.equal(workers.length, 1, 'viewport changes must retain the worker and GPU context');
        assert.deepEqual(worker.messages.map(message => message.type), ['start', 'cancel', 'start']);
        assert.equal(worker.messages[1].jobId, jobId);
        assert.notEqual(worker.messages[2].jobId, jobId);
        assert.equal(worker.terminated, undefined);
        // Rapid viewport changes must reject old frames, failures, and completion
        // messages while the new job remains active on the same worker.
        worker.receive({ type: 'frame', jobId, bitmap });
        worker.receive({ type: 'error', jobId, message: 'stale certification failure' });
        worker.receive({ type: 'complete', jobId });
        assert.equal(draws.length, 1);
        assert.equal(worker.terminated, undefined);
        assert.equal(worker.messages[2].snapshot.viewport.centerRe, '2');
        cancelPlanarDomainDynamics();
