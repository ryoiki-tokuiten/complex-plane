...
test('drawn-cut crossings are stable at polyline vertices', () => {
    ...
    const crossing = branchCutCrossingForSegment(
...
test('continued logarithm changes by one sheet increment', () => {
    const baseState = { logBase: { re: Math.E, im: 0 }, branchCutType: 'ray', branchCutAngle: Math.PI };
    const principal = evaluateOnSheet('ln', { re: 1, im: 0 }, 0, baseState);
    const next = evaluateOnSheet('ln', { re: 1, im: 0 }, 1, baseState);
...
test('continued logarithm uses the selected ray as its argument boundary', () => {
    const runtimeState = { logBase: { re: Math.E, im: 0 }, branchCutType: 'ray', branchCutAngle: 0 };
    const value = evaluateOnSheet('ln', { re: 0, im: 1 }, 0, runtimeState);
    assert.ok(Math.abs(value.im + Math.PI * 1.5) < 1e-10);
});

test('continued inverse sine alternates reflected sheets', () => {
    const runtimeState = { branchCutType: 'ray', branchCutAngle: Math.PI };
    const z = { re: 0.25, im: 0.4 };
    const principal = evaluateOnSheet('asin', z, 0, runtimeState);
    const adjacent = evaluateOnSheet('asin', z, 1, runtimeState);
...
test('continued algebraic maps preserve custom input expressions and recursive c', () => {
    ...
    const value = evaluateOnSheet('algebraic_chaining', { re: 2, im: 0 }, 0, runtimeState);
...
test('continued algebraic custom expressions use the active sheet', () => {
    ...
    const principal = evaluateOnSheet('algebraic_chaining', { re: 1, im: 0 }, 0, runtimeState);
    const next = evaluateOnSheet('algebraic_chaining', { re: 1, im: 0 }, 1, runtimeState);
