...
static int ce_write_decimal(char *output, uint32_t capacity, const mpfr_t value, size_t digits) {
    mpfr_exp_t exponent = 0;
    char *mantissa = mpfr_get_str(NULL, &exponent, 10, digits, value, MPFR_RNDN);
    if (!mantissa) return 0;
    const int negative = mantissa[0] == '-';
    const char *number = mantissa + negative;
    const size_t number_length = strlen(number);
    char exponent_digits[32];
    size_t exponent_length = 0;
    long scientific_exponent = (long)exponent - 1l;
    unsigned long magnitude = scientific_exponent < 0
        ? (unsigned long)(-scientific_exponent)
        : (unsigned long)scientific_exponent;
    do {
        exponent_digits[exponent_length++] = (char)('0' + magnitude % 10u);
        magnitude /= 10u;
    } while (magnitude && exponent_length < sizeof(exponent_digits));

    const size_t required = (size_t)negative + 1u + (number_length > 1u ? 1u + number_length - 1u : 0u) +
        1u + (scientific_exponent < 0 ? 1u : 0u) + exponent_length + 1u;
    ...
    output[cursor++] = number[0];
    if (number_length > 1u) {
        output[cursor++] = '.';
        memcpy(output + cursor, number + 1, number_length - 1u);
        cursor += number_length - 1u;
    }
    output[cursor++] = 'e';
    if (scientific_exponent < 0) output[cursor++] = '-';
    while (exponent_length) output[cursor++] = exponent_digits[--exponent_length];
    output[cursor] = '\0';
    mpfr_free_str(mantissa);
    return 1;
}
...
int32_t ce_precise_pixel_coordinate(const char *center_re, const char *center_im,
    ...
    mpfr_t real_center, imaginary_center, x_span, y_span, scale, scratch, real, imaginary;
    mpfr_inits2(precision, real_center, imaginary_center, x_span, y_span,
                scale, scratch, real, imaginary, (mpfr_ptr)0);

    int32_t status = 0;
    if (!ce_set_decimal(real_center, center_re) || !ce_set_decimal(imaginary_center, center_im)) {
        status = -2;
        goto done;
    }
    ...
    ce_pixel_coordinate(real, real_center, x_span, frame_width, pixel_x, 0, scratch);
    ce_pixel_coordinate(imaginary, imaginary_center, y_span, frame_height, pixel_y, 1, scratch);

    const size_t decimal_digits = (size_t)ceil((double)precision * 0.30102999566398119521) + 2u;
    if (!ce_write_decimal(output_re, output_re_capacity, real, decimal_digits) ||
        !ce_write_decimal(output_im, output_im_capacity, imaginary, decimal_digits)) {
        status = -3;
    }

done:
    mpfr_clears(real_center, imaginary_center, x_span, y_span,
                scale, scratch, real, imaginary, (mpfr_ptr)0);
    return status;
}
