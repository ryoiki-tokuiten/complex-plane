...
static int pc_viewport_set(ce_precise_viewport *viewport,
                           ...
                           const char *x_span, const char *y_span, uint32_t width, uint32_t height) {
    if (!width || !height || !ce_set_decimal(viewport->center_re, center_re) ||
        !ce_set_decimal(viewport->center_im, center_im) || !ce_set_decimal(viewport->x_span, x_span) ||
        !ce_set_decimal(viewport->y_span, y_span) || mpfr_sgn(viewport->x_span) <= 0 ||
        mpfr_sgn(viewport->y_span) <= 0) return 0;
...
void *ce_precision_image_context_create(const ce_map_config *config,
    ...
    if (!config || !ce_precision_valid(precision_bits) ||
    ...
    if (!pc_viewport_set(&context->viewport, view_center_re, view_center_im,
                         view_x_span, view_y_span, view_width, view_height)) {
...
int32_t ce_project_precise_pixels(const ce_map_config *config,
    ...
    if (!pc_viewport_set(&input_viewport, input_center_re, input_center_im,
                         input_x_span, input_y_span, input_width, input_height) ||
        !pc_viewport_set(&output_viewport, output_center_re, output_center_im,
                         output_x_span, output_y_span, output_width, output_height)) {
