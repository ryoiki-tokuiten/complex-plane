...
function RangeControls({ controls, labelClass = '' }) {
  return controls.map(([id, label, outputId, value, min, max, step, tooltip]) =>
    <RangeControl key={id} id={id} name={id} label={label}
      labelClass={labelClass} outputId={outputId} outputValue={value}
      min={min} max={max} step={step} value={value} data-tooltip={tooltip} />
  );
}

export function Overlays({ contextPanel = '' }) {
  return (
    <>
      {!contextPanel && <Ui as="div" id={"frontend_modal_root"} />}
      {contextPanel === 'domain' && <Ui as="div" id={"domain_coloring_context_panel"}>
        <Ui as="div" id={"domain_coloring_options_div"} class={"dependent-settings-block"}>
          <div class={"viz-settings-grid"}>
            <div class={"domain-palette-selector-container"}>
              <div class={"domain-palette-header"}>
                <span class={"domain-palette-selector-label domain-palette-label"}>
                  ...
                  <Ui as="span" id={"active_domain_palette_name"} class={"active-palette-label"} />
                </span>
                <Ui as="button" id={"view_palette_circle_btn"} class={"view-palette-circle-btn"} type={"button"} data-tooltip={"View color guide circle for active palette"}>
                  View Color Circle
                </Ui>
              </div>
              <Ui as="div" class={"domain-palette-circles"} id={"domain_palette_circles"} />
            </div>
            <Ui as="div" class={"control-group"} id={"orbit_coloring_mode_group"}>
              <label for={"orbit_coloring_mode_select"} class={"compact-text"}>Orbit Coloring:</label>
              <Ui as="select" id={"orbit_coloring_mode_select"} class={"control-select"} data-tooltip={"Choose which orbit observable colors recursive output chains"}>
            ...
            <div class={"control-group viz-control-span-2"}>
              <label for={"show_domain_coloring_key_cb"} class={"slider-label"}>
                <Ui as="input" type={"checkbox"} id={"show_domain_coloring_key_cb"} />
      ...
      {contextPanel === 'manifold' && <Ui as="div" id={"manifold_context_panel"}>
        <Ui as="div" id={"manifold_options_div"} class={"dependent-settings-block viz-stacked-options"}>
          <div class={"control-group"} style={"margin-bottom: 8px;"}>
            ...
            <Ui as="select" id={"manifold_shape_selector"} class={"control-select"} data-tooltip={"Choose the 3D manifold embedding geometry"}>
          ...
          <div class={"viz-toggle-grid viz-toggle-grid--compact viz-toggle-grid--single"}>
            <label for={"enable_manifold_transformation_cb"} class={"viz-toggle-card viz-toggle-card--compact"}>
              <Ui as="input" type={"checkbox"} id={"enable_manifold_transformation_cb"} data-tooltip={"Show continuous morph animation from flat plane into 3D manifold"} />
              <span class={"custom-checkbox-visual"} />
          ...
          <Ui as="div" id={"manifold_surface_options_div"} class={"dependent-settings-block viz-subdetail-block"}>
      ...
      {contextPanel === 'riemann' && <Ui as="div" id={"riemann_surface_context_panel"}>
        <Ui as="div" id={"riemann_surface_options_div"} class={"dependent-settings-block"}>
          <div class={"viz-settings-grid"}>
            <div class={"control-group"}>
              <label for={"riemann_surface_component_selector"}>Surface Height</label>
              <Ui as="select" id={"riemann_surface_component_selector"} class={"control-select"} data-tooltip={"Choose the component of w used as surface height"}>
            ...
            <RangeControls controls={RIEMANN_RANGES} />
            <div class={"control-group"}>
              <label for={"riemann_surface_wireframe_cb"}>
                <Ui as="input" type={"checkbox"} id={"riemann_surface_wireframe_cb"} checked />
                <span class={"custom-checkbox-visual"} />
                Surface Grid
              </label>
            </div>
            <div class={"control-group"}>
              <label for={"riemann_surface_contours_cb"} data-tooltip={"Overlay height level contour lines onto the surface"}>
                <Ui as="input" type={"checkbox"} id={"riemann_surface_contours_cb"} />
                <span class={"custom-checkbox-visual"} />
                Contour Lines
              </label>
            </div>
            <Ui as="div" id={"riemann_surface_contours_details"} class={"contour-controls-panel contour-controls-panel--grid viz-control-span-2 hidden"}>
              <div class={"control-group contour-slider-field"}>
                <div class={"contour-slider-label"}>
                  ...
                  <Ui as="output" id={"riemann_surface_contour_interval_value_display"}>
                    0.50
                  </Ui>
                </div>
                <div class={"slider-container"}>
                  <Ui as="input" type={"range"} id={"riemann_surface_contour_interval_slider"} min={"0.05"} max={"3.00"} step={"0.05"} value={"0.50"} />
                </div>
              </div>
              <div class={"control-group contour-slider-field"}>
                <div class={"contour-slider-label"}>
                  <label for={"riemann_surface_contour_thickness_slider"}>Contour Thickness:</label>
                  <Ui as="output" id={"riemann_surface_contour_thickness_value_display"}>
                    1.5
                  </Ui>
                </div>
                <div class={"slider-container"}>
                  <Ui as="input" type={"range"} id={"riemann_surface_contour_thickness_slider"} min={"0.5"} max={"5.0"} step={"0.1"} value={"1.5"} />
                </div>
              </div>
              <div class={"contour-action-row"}>
                <Ui as="button" id={"riemann_surface_show_2d_contour_btn"} class={"contour-2d-toggle-btn"} type={"button"}>
                  <Icon name="image" />
          ...
          <div class={"riemann-surface-actions"}>
            <Ui as="button" id={"riemann_surface_reset_view_btn"} type={"button"}>
              Reset View
            </Ui>
          </div>
        </Ui>
      </Ui>}
      {contextPanel === 'taylor' && <Ui as="div" id={"taylor_context_panel"}>
        <Ui as="div" id={"taylor_series_options_detail_div"} class={"taylor-series-panel"}>
          <div class={"taylor-series-header"}>
            <div class={"taylor-series-header-copy"}>
              <h4 class={"taylor-series-title"}>Expand Around z0</h4>
            </div>
            <Ui as="div" id={"taylor_series_center_status"} class={"taylor-series-center-status"}>
              z0 = 0
            </Ui>
          </div>
          <RangeControl id="taylor_series_order_slider" name="taylor_series_order_slider"
          ...
          <Ui as="div" id={"taylor_series_custom_center_inputs_div"} class={"control-group taylor-series-card taylor-series-custom-block"}>
            <Ui as="div" id={"taylor_complex_points_ui_container"} />
          </Ui>
          <div class={"control-group taylor-series-card taylor-pick-center-card"}>
            <Ui as="button" type={"button"} id={"pick_taylor_center_canvas_btn"} class={"taylor-pick-center-btn"} data-tooltip={"Click anywhere on the z or w plane canvas to pick that point as the expansion center"}>
              <Ui as="span" id={"pick_taylor_center_btn_text"}>
      ...
      {!contextPanel && <>
      <Ui as="div" id={"dynamic_plotting_root"} />
      <Ui as="div" id={"domain_palette_circle_panel"} class={"dynamic-plotting-controls hidden"} role={"dialog"} aria-modal={"false"} aria-label={"Domain Coloring Guide"} />
      <Ui as="div" id={"real_plots_palette_circle_panel"} class={"dynamic-plotting-controls hidden"} role={"dialog"} aria-modal={"false"} aria-label={"Real Plots Color Guide"} />
      </>}
    </>
  );
}
