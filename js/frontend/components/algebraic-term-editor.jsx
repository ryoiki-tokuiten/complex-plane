        ...
        <div class="algebraic-slider-row">
            ...
            <div class="algebraic-slider-container">
                <input type="range" min="-5" max="5" step="0.1" value={value}
                    onInput={event => onValue(Number.parseFloat(event.currentTarget.value))} />
            </div>
        </div>
    );
}
    ...
    return (
        <label class="algebraic-checkbox-label">
            <input type="checkbox" checked={Boolean(factor[name])} onChange={event => {
                mutate(() => { factor[name] = event.currentTarget.checked; }, `algebraicChainingTerms.factor.${name}`);
            }} />
                    ...
                    {factor.func !== 'c' && (
                        <div class="algebraic-factor-detail-row">
                            ...
                            <FunctionSelect value={factor.chainedFunc || 'none'} onChange={value => {
                                mutate(() => { factor.chainedFunc = value; }, 'algebraicChainingTerms.factor.chainedFunc');
                            }} />
                        </div>
                    )}
                    <AlgebraicRange label="Power " value={factor.power ?? 1} onValue={value => {
                        mutate(() => { factor.power = value; }, 'algebraicChainingTerms.factor.power');
                    }} />
        ...
        <div class="algebraic-term-card">
            <div class="algebraic-term-header">
                ...
                {termCount > 1 && <button type="button" class="algebraic-term-remove-btn" onClick={() => {
                    mutate(terms => terms.splice(index, 1), 'algebraicChainingTerms');
                }}>✕ Remove</button>}
            </div>
            <div class="algebraic-coeff-grid">
                <AlgebraicRange label="Re coeff " value={coefficient.re} onValue={value => {
                    mutate(() => {
                        term.coeff ||= { re: 0, im: 0 };
                        term.coeff.re = value;
                    }, 'algebraicChainingTerms.coeff.re');
                }} />
                <AlgebraicRange label="Im coeff " value={coefficient.im} onValue={value => {
                    mutate(() => {
                        term.coeff ||= { re: 0, im: 0 };
                        term.coeff.im = value;
                    }, 'algebraicChainingTerms.coeff.im');
                }} />
            </div>
